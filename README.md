...
- The method text is now automatically dosed from the model-generated daily loads.
- A key-method trigger is used: by default a component becomes a key focus when its daily load is >= 40% of Tref.
- The selected database method provides the type/context, while the final minutes, repetitions and interval structure are generated from the daily component loads.
- Old shooting text from source training plans is filtered out in the physical-only method rendering.
