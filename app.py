...
with tab_micro:
    ...
    if not daily_plan_df.empty:
        ...
        cols = [
            "week", "day", "date", "day_name", "main_focus", "method", "method_id",
            "session_description",
        ] + [cid for cid in components_valid["id"].tolist() if cid in daily_plan_df.columns] + [
            "total_real_min", "readiness_before_main", "readiness_after_main", "warnings"
        ]
        cols = [c for c in cols if c in daily_plan_df.columns]
        st.dataframe(daily_plan_df[cols], use_container_width=True, height=420)

        st.markdown("### Конкретни тренировки от базата")
        for _, r in daily_plan_df.iterrows():
            title = f"Ден {int(r['day'])} — {r['day_name']} | {r['main_focus']} | {r['method']}"
            with st.expander(title, expanded=int(r['day']) <= 2):
                st.write(r.get("session_description") or r.get("method_notes") or "Няма описание.")
                zone_cols = [cid for cid in components_valid["id"].tolist() if cid in daily_plan_df.columns]
                zone_line = ", ".join([f"{cid}: {float(r.get(cid, 0.0)):g} мин" for cid in zone_cols if float(r.get(cid, 0.0)) > 0])
                st.caption(zone_line if zone_line else "Няма планиран товар по компоненти.")
