...
from methods_library import load_methods
...
with st.sidebar:
    ...
    total_weeks = st.slider("Брой седмици до основния старт", 4, 40, int(DEFAULT_MODEL_PARAMS["total_weeks"]), 1)
    initial_weekly_volume_hours = st.number_input("Начален седмичен обем (часове)", 2.0, 40.0, float(DEFAULT_MODEL_PARAMS["initial_weekly_volume_hours"]), 0.5)

    st.header("Прогресия и пик")
    monthly_progression_rate = st.slider("Месечна прогресия", 0.00, 0.12, float(DEFAULT_MODEL_PARAMS["monthly_progression_rate"]), 0.005, format="%.3f")
    peak_fraction = st.slider("Пик на общия товар като част от периода", 0.30, 0.75, float(DEFAULT_MODEL_PARAMS["peak_fraction"]), 0.01)
    post_peak_reduction = st.slider("Редукция след пика", 0.00, 0.35, float(DEFAULT_MODEL_PARAMS["post_peak_reduction"]), 0.01)

    st.header("Мезоцикъл и акценти")
    mesocycle_len = st.selectbox("Дължина на мезоцикъл", [3, 4, 5], index=1)
    max_accents = st.slider("Максимум акценти в мезоцикъл", 1, 5, int(DEFAULT_MODEL_PARAMS["max_accents"]), 1)
    wave_amplitude = st.slider("Амплитуда на компонентната вълна", 0.00, 0.60, float(DEFAULT_MODEL_PARAMS["wave_amplitude"]), 0.01)

    st.header("Стрес прагове")
    intro_stress = st.slider("Вработваща седмица — акцент", 1.00, 1.45, float(DEFAULT_MODEL_PARAMS["intro_stress"]), 0.01)
    dev1_stress = st.slider("Развиваща 1 — акцент", 1.05, 1.65, float(DEFAULT_MODEL_PARAMS["dev1_stress"]), 0.01)
    dev2_stress = st.slider("Развиваща 2 — акцент", 1.05, 1.75, float(DEFAULT_MODEL_PARAMS["dev2_stress"]), 0.01)
    maintenance_stress = st.slider("Поддържащ стрес", 0.75, 1.10, float(DEFAULT_MODEL_PARAMS["maintenance_stress"]), 0.01)
    recovery_stress = st.slider("Възстановителен стрес", 0.50, 0.95, float(DEFAULT_MODEL_PARAMS["recovery_stress"]), 0.01)
    risk_limit = st.slider("Горна рискова граница", 1.20, 1.90, float(DEFAULT_MODEL_PARAMS["risk_limit"]), 0.01)
...
# ------------------------------ Tabs ------------------------------
tab_settings, tab_model, tab_calendar, tab_weekly, tab_micro, tab_export = st.tabs(
    ["1) Основни", "2) Настройки на модела", "3) Календар", "4) Седмична динамика", "5) Микроцикъл", "6) Експорт"]
)
...
with tab_settings:
    ...
    if st.button("Пропорционално нагласи компонентите към началния седмичен обем", type="secondary"):
    ...
    st.info("Съвет: за първи тест натисни бутона за пропорционално нагласяне към 12 часа, после виж графиките.")
...
with tab_model:
    ...
    with c1:
        params["spill_threshold"] = st.slider("Праг за разлив от Tref", 0.10, 0.90, float(DEFAULT_MODEL_PARAMS["spill_threshold"]), 0.05)
        params["spill_down_percent"] = st.slider("Разлив надолу към по-ниска съседна зона", 0.00, 0.50, float(DEFAULT_MODEL_PARAMS["spill_down_percent"]), 0.01)
        params["spill_up_percent"] = st.slider("Разлив нагоре към по-висока съседна зона", 0.00, 0.50, float(DEFAULT_MODEL_PARAMS["spill_up_percent"]), 0.01)
        params["drop_sensitivity"] = st.slider("Чувствителност на readiness към товар", 20.0, 120.0, float(DEFAULT_MODEL_PARAMS["drop_sensitivity"]), 1.0)
    with c2:
        params["camp_load_bonus"] = st.slider("Бонус натоварване при лагер", 0.00, 0.40, float(DEFAULT_MODEL_PARAMS["camp_load_bonus"]), 0.01)
        params["camp_accent_bonus"] = st.slider("Доп. бонус за акцент при лагер", 0.00, 0.30, float(DEFAULT_MODEL_PARAMS["camp_accent_bonus"]), 0.01)
        params["post_camp_reduction"] = st.slider("Редукция след лагер", 0.00, 0.50, float(DEFAULT_MODEL_PARAMS["post_camp_reduction"]), 0.01)
    with c3:
        params["taper_length_weeks"] = st.slider("Тейпър седмици", 1, 4, int(DEFAULT_MODEL_PARAMS["taper_length_weeks"]), 1)
        params["taper_reduction_week_1"] = st.slider("Тейпър редукция седмица 1", 0.00, 0.70, float(DEFAULT_MODEL_PARAMS["taper_reduction_week_1"]), 0.01)
        params["taper_reduction_week_2"] = st.slider("Тейпър редукция седмица 2", 0.00, 0.80, float(DEFAULT_MODEL_PARAMS["taper_reduction_week_2"]), 0.01)
        params["high_intensity_preservation"] = st.slider("Запазване на интензивността", 0.30, 1.00, float(DEFAULT_MODEL_PARAMS["high_intensity_preservation"]), 0.01)
...
# Shared generation
try:
    components_valid = validate_components(st.session_state.components_df)
...
with tab_micro:
    ...
    selected_week = st.selectbox("Избери седмица", options=list(range(1, total_weeks + 1)), index=0)
    methods = load_methods("training_methods.csv")
    ...
    week_start = start_date + timedelta(days=(selected_week - 1) * 7)
    if not daily_plan_df.empty:
        daily_plan_df["date"] = [week_start + timedelta(days=i) for i in range(7)]
        cols = ["week", "day", "date", "day_name", "main_focus", "method"] + [cid for cid in components_valid["id"].tolist() if cid in daily_plan_df.columns] + ["total_real_min", "readiness_before_main", "readiness_after_main", "method_notes", "warnings"]
        st.dataframe(daily_plan_df[cols], use_container_width=True, height=360)

        st.markdown("### Readiness динамика")
        fig_readiness = px.line(
            ...
            color="component_id",
            markers=True,
            hover_data=["component", "day_name", "real_day_load", "effective_day_load"],
            title="Readiness след дневното натоварване",
        )
        ...
        st.markdown("### Планирано в микроцикъла спрямо седмичната цел")
        comparison_df = compare_weekly_target_vs_plan(weekly_plan_df[weekly_plan_df["week"] == selected_week], daily_plan_df)
        fig_compare = px.bar(
            comparison_df,
            x="component_id",
            y=["target_weekly_load", "microcycle_planned_load"],
            barmode="group",
            title="Седмична цел срещу разпределение в микроцикъла",
        )
...
with tab_export:
    ...
    selected_week_export = st.selectbox("Седмица за експорт на микроцикъл", options=list(range(1, total_weeks + 1)), index=0, key="export_week")
    daily_plan_export, readiness_export, warnings_export = generate_microcycle(
        selected_week_export,
        weekly_plan_df,
        components_valid,
        params=params,
        methods=load_methods("training_methods.csv"),
    )
    comparison_export = compare_weekly_target_vs_plan(weekly_plan_df[weekly_plan_df["week"] == selected_week_export], daily_plan_export)
    ...
    st.info("Ако искаш да добавиш реална база от тренировки, създай `training_methods.csv` със структурата от README.md.")
