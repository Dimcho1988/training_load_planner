...
def calculate_effective_day_load(
    ...
    spill_down_percent: float = 0.20,
    spill_up_percent: float = 0.10,
    include_base: bool = True,
) -> Dict[str, float]:
    """Calculate effective physiologic daily load.

    It adds virtual baseline, a one-way cascade from higher to lower zones,
    and threshold-based spillover to adjacent zones. The cascade is normally
    100% downward inclusion. Spillover is asymmetric: by default 20% downward
    and 10% upward, calculated only on the excess above threshold.
    """
    ...
    if tref_weekly:
        for idx, cid in enumerate(ZONE_ORDER):
            ...
            if daily_load > threshold_load:
                ...
                spill_down = excess * float(spill_down_percent)
                spill_up = excess * float(spill_up_percent)
                if idx > 0:
                    # Downward adjacent spill: overloaded zone additionally stresses the lower neighbor.
                    effective[ZONE_ORDER[idx - 1]] = effective.get(ZONE_ORDER[idx - 1], 0.0) + spill_down
                if idx < len(ZONE_ORDER) - 1:
                    # Upward adjacent spill: when low-threshold fibres fatigue, higher-threshold fibres join in.
                    effective[ZONE_ORDER[idx + 1]] = effective.get(ZONE_ORDER[idx + 1], 0.0) + spill_up
...
def calculate_effective_weekly_load(
    ...
    eff_daily = calculate_effective_day_load(
        ...
        spill_down_percent=spill_down_percent,
        spill_up_percent=spill_up_percent,
        include_base=True,
    )
