...
def apply_fatigue_spillover(
    ...
    """Additional asymmetric fatigue spillover after very large daily load.

    Downward spill to the lower neighboring zone defaults to 20% of excess.
    Upward spill to the higher neighboring zone defaults to 10% of excess.
    """
    ...
    for idx, cid in enumerate(ZONE_ORDER):
        ...
        if idx > 0:
            ...
            spill_load_down = excess * float(spill_down_percent)
            spill_drop_down = (spill_load_down / tref) * float(drop_sensitivity)
            updated[target] = clamp(updated.get(target, 100.0) - spill_drop_down)
        if idx < len(ZONE_ORDER) - 1:
            target = ZONE_ORDER[idx + 1]
            spill_load_up = excess * float(spill_up_percent)
            spill_drop_up = (spill_load_up / tref) * float(drop_sensitivity)
            updated[target] = clamp(updated.get(target, 100.0) - spill_drop_up)
    return updated
