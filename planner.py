...
def generate_microcycle(
    ...
    for day_idx in range(7):
        ...
        # Determine dominant component and method.
        dominant = max(adjusted_load, key=lambda c: adjusted_load.get(c, 0.0)) if adjusted_load else "Z1"
        if adjusted_load.get(dominant, 0.0) <= 0:
            dominant = "Z1"
        method = select_best_method(dominant, phase_fraction, readiness_before.get(dominant, 100.0), methods, target_loads=adjusted_load)
