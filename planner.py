...
def choose_daily_focus(
    ...
    The largest volume is often Z1/Z2, but the real methodological focus may be
    Z3/Z4/Z5, Z6 or strength. Therefore the score weights the planned load by
    the component specificity and by whether the component is a weekly accent.
    This prevents every day from becoming "Z2" just because Z2 minutes are high.
    """
    ...
    for cid, raw_load in adjusted_load.items():
        ...
        # Ignore tiny high-intensity crumbs unless they are the only meaningful load.
        ...
        specificity_weight = (spec + 1.0) ** 1.35
        accent_weight = 1.75 if is_accent else 1.0
        readiness_weight = 0.35 if readiness < 60 else (0.70 if readiness < 80 else 1.0)

        # Z1/Z2 are allowed as focus, but they should not hide meaningful quality work.
...
def generate_microcycle(
    ...
    for day_idx in range(7):
        ...
        dominant = choose_daily_focus(adjusted_load, week_plan, readiness_before)
        ...
        rendered_method_text = render_method_text(method, adjusted_load)
