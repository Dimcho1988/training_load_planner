...
from methods_library import FALLBACK_METHODS, select_best_method
...
def generate_microcycle(
    ...
    for day_idx in range(7):
        ...
        if adjusted_load.get(dominant, 0.0) <= 0:
            dominant = "Z1"
        method = select_best_method(dominant, phase_fraction, readiness_before.get(dominant, 100.0), methods)
