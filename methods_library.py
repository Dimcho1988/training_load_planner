...
def render_method_text(method: Mapping, target_loads: Mapping[str, float] | None = None) -> str:
    """Render a method description with the planned daily component loads.

    The database can contain explicit placeholders such as {Z1_min}, {Z3_min},
    {OSI_min}, and {total_min}. Older free-text templates often contain generic
    placeholders like {мин}, {време}, {часове}, {повторения}, {сек}. These are
    converted to practical planning hints rather than left empty.
    """
    text = str(method.get("template_text") or method.get("description") or "").strip()
    target_loads = target_loads or {}
    cleaned = {cid: round(float(target_loads.get(cid, 0.0) or 0.0), 1) for cid in COMPONENT_IDS}
    total = round(sum(cleaned.values()), 1)
    planned_parts = [f"{cid}: {minutes:g} мин" for cid, minutes in cleaned.items() if minutes > 0]
    planned_summary = "Планови обеми за деня: " + (", ".join(planned_parts) if planned_parts else "няма зададен товар") + "."

    # Explicit component placeholders.
    values = {f"{cid}_min": cleaned[cid] for cid in COMPONENT_IDS}
    values["total_min"] = total
    for key, value in values.items():
        text = text.replace("{" + key + "}", str(value))

    primary = str(method.get("primary_component") or method.get("component") or "Z1")
    primary_min = cleaned.get(primary, 0.0)
    endurance_min = sum(cleaned.get(cid, 0.0) for cid in ["Z1", "Z2", "Z3", "Z4", "Z5", "Z6"])

    # Estimate repetitions for interval-style sessions.
    reps = 5
    if primary in {"Z3", "Z4"} and primary_min > 0:
        reps = max(2, min(8, round(primary_min / 6)))
    elif primary in {"Z5", "Z6"} and primary_min > 0:
        reps = max(4, min(12, round(primary_min / 2)))
    interval_min = round(primary_min / reps, 1) if reps and primary_min > 0 else "?"

    generic_replacements = {
        "{време}": _format_min(endurance_min or total),
        "{часове}": round((endurance_min or total) / 60, 1),
        "{мин}": "според плановите минути",
        "{повторения}": reps,
        "{интервал_мин}": interval_min,
        "{сек}": "10–20",
        "{дължина}": "подходяща дистанция",
    }
    for key, value in generic_replacements.items():
        text = text.replace(key, str(value))

    if not text:
        text = str(method.get("description") or method.get("title") or "Тренировка по избрания компонент.")

    return f"{planned_summary}\n\nМетод от базата: {text}"
